...
public class AlignToNet extends Command {
    ...
    public AlignToNet(DriveSubsystem subsystem) {
        ...
        xController = new ProfiledPIDController(DriveConstants.DriveToNetP, DriveConstants.DriveToNetI, DriveConstants.DriveToNetD, new Constraints(1.5, 2.5));
    ...
    public void initialize() {
        xController.reset(new State(m_robotDrive.getPose().getX(), m_robotDrive.getFieldRelSpeeds().vxMetersPerSecond * (Helpers.isBlue ? 1 : -1)));
    ...
    public void execute() {
        ...
        m_robotDrive.driveWithController(MathUtil.clamp(-0.2, driveOutput, 0.2), rotOutput, true);
    }
